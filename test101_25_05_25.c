...
int main() {
    ...
    Player player = {10, 10, 10, 10, 0, 1, 100, NULL};
...
void battle(Player* player) {
    ...
    while (monsterHP > 0 && player->hp > 0) {
        printf("\n--- Status ---\n");
        printf("👤 Speler - HP: %d | Damage: %d | Level: %d | Experience: %d\n", player->hp, player->damage, player->level, player->experience);
        printf("👹 Monster - HP: %d\n", monsterHP);

        // Monster kiest willekeurig een aanval
        int attackType = rand() % 2;
        if (attackType == 0 && (monsterAttack & ATTACK_FIRE)) {
            int damage = 8 - player->defense;
            if (damage < 0) damage = 0;
            player->hp -= damage;
            printf("🔥 Monster gebruikt vuur! (-%d HP)\n", damage);
        } else if (attackType == 1 && (monsterAttack & ATTACK_SLASH)) {
            int damage = 5 - player->defense;
            if (damage < 0) damage = 0;
            player->hp -= damage;
            printf("🗡️  Monster valt met zwaard aan! (-%d HP)\n", damage);
        }
    ...
    if (player->hp > 0) {
        ...
        if (player->experience >= calculateExpForNextLevel(player->level)) {
            levelUp(player);
        }
...
int calculateExpForNextLevel(int currentLevel) {
    return 100 * (int)pow(1.25, currentLevel);
}